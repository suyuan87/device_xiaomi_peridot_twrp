# 橙 狐 (OFRP) for Redmi K60 (peridot)  
...
# 如何使用
进入[Release](https://github.com/ymdzq/OFRP-device_xiaomi_peridot/releases)中，点开Assets选项，点击zip压缩包文件名下载  
...
例如OFRP源代码根目录为~/fox_12.1，则保存为~/fox_12.1/device/xiaomi/peridot/:  
```bash
cd ~/fox_12.1
mkdir -p device/xiaomi
cd device/xiaomi
git clone https://github.com/ymdzq/OFRP-device_xiaomi_peridot.git peridot
```
...
打开源代码根目录运行:  
```bash
. build/envsetup.sh && lunch twrp_peridot-eng && mka recoveryimage
```
# 云编译
...
`https://github.com/ymdzq/OFRP-device_xiaomi_peridot`  
